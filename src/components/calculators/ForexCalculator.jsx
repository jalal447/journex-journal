...
export default function ForexCalculator() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    const { data } = await calculatorsApi.forex(form);
    setResult(data);
    toastSuccess("Forex calculation completed.");
  };

  return (
    <Card>
      <p className="mb-3 font-semibold">Forex Position Size</p>
      ...
      {result ? (
        <div className="mt-3 text-sm text-white/80">
          ...
          <p>Stop Loss Distance: {result.stop_loss_distance}</p>
          <p>Lots: {result.position_size_lots}</p>
        </div>
      ) : null}
    </Card>
  );
